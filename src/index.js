...
function initSignaturePad(id) {
  const problems = document.getElementById(id).getElementsByClassName('problem');
  for (var i=0; i<problems.length; i++) {
    let signaturePads = [];
    const drawElements = problems[i].getElementsByTagName('CANVAS');
    for (var j=0; j<drawElements.length; j++) {
      const signaturePad = new SignaturePad(drawElements[j], {
        minWidth: 5,
        maxWidth: 5,
        penColor: 'white',
      ...
      signaturePad.onEnd = function() {
        prediction(this._canvas);
      };
    }
    const clearElement = problems[i].getElementsByClassName('clear');
    for (var j=0; j<signaturePads.length; j++) {
      (function(signaturePad) {
      clearElement[j].addEventListener('click', function() {
        signaturePad.clear();
        prediction(signaturePad._canvas);
      });
      }(signaturePads[j]));
    }
    if (i == 0) {  // cache memory
      prediction(signaturePads[0]._canvas);
    }
...
function prediction(drawElement) {
  const drawElements = drawElement.parentNode.parentNode.getElementsByTagName('CANVAS');
  const c = drawElement.parentNode.parentNode.parentNode.getElementsByClassName('c')[0];
  if (c) {
    let result = '';
    for (var i=0; i<drawElements.length; i++) {
      const imageData = getImageData(drawElements[i]);
      // 何も入力していない状態は無視
      let sum = 0;
      ...
      if (sum != 0) {
        ...
        if (accuracyScores[maxAccuracy] > 0.5) {
          result += maxAccuracy.toString();
        }
      }
    }
    c.innerText = result;
